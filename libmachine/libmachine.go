...
func (c *Client) NewHost(driver drivers.Driver) (*host.Host, error) {
	certDir := filepath.Join(c.Path, "certs")
	...
	hostOptions := &host.Options{
		AuthOptions: &auth.Options{
			...
			ServerCertPath:   filepath.Join(c.GetMachinesDir(), "server.pem"),
			ServerKeyPath:    filepath.Join(c.GetMachinesDir(), "server-key.pem"),
		},
...
func (c *Client) Create(h *host.Host) error {
